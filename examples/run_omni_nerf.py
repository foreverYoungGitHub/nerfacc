...
from datasets.st3d import load_st3d_data
...
def render_path(rays, hw, render_kwargs, savedir=None, render_factor=0):
    ...
    for i in tqdm(range(rays_o.shape[0] // batch)):
        # print(i, time.time() - t)
        t = time.time()
        rgb, dep, grad, _ = render(
            H,
            W,
            rays=[
                rays_o[i * batch : (i + 1) * batch],
                rays_d[i * batch : (i + 1) * batch],
            ],
            **render_kwargs,
        )
        # if i==0:
...
def render_rays(
    ...
    rgb_map, depth_map, grad_map, weights = raw2outputs(
    ...
    if N_importance > 0:
        ...
        z_samples = sample_pdf(
            z_vals_mid, weights[..., 1:-1], N_importance, det=True, pytest=False
        )
    ...
    for k in ret:
        if (
            DEBUG
            and ret[k]
            and (torch.isnan(ret[k]).any() or torch.isinf(ret[k]).any())
        ):
...
def config_parser():
    ...
    ## st3d flags
    parser.add_argument(
        "--use_depth", action="store_true", help="use depth to update"
    )
    parser.add_argument(
        "--use_gradient", action="store_true", help="use gradient to update"
    )
    parser.add_argument(
        "--stage",
        type=int,
...
def train():
    ...
    if args.dataset_type == "st3d":
        scene_dir = os.path.join(args.datadir, args.dataset_type, scene)
        rays_o, rays_d, rays_g, rays_rgb, rays_depth, hw = load_st3d_data(
            scene_dir, args.stage
        )
    ...
    basedir = args.basedir
    expname = args.expname
    os.makedirs(os.path.join(basedir, expname, scene), exist_ok=True)
    f = os.path.join(basedir, expname, scene, "args.txt")
    with open(f, "w") as file:
        for arg in sorted(vars(args)):
            attr = getattr(args, arg)
            file.write("{} = {}\n".format(arg, attr))
    if args.config is not None:
        f = os.path.join(basedir, expname, scene, "config.txt")
        with open(f, "w") as file:
        ...
        with torch.no_grad():
            if args.stage > 0:
                testsavedir = os.path.join(
                    ...
                    expname,
                    scene,
                    "renderonly_stage_{}_{:06d}".format(args.stage, start),
                )
            else:
                testsavedir = os.path.join(
                    basedir,
                    expname,
                    scene,
                    "renderonly_train_{}_{:06d}".format(
            ...
            gt_loss = img2mse(
                torch.tensor(rgbs[-1]), torch.tensor(rays_rgb_test[-1])
            )
            ...
            imageio.mimwrite(
                os.path.join(testsavedir, "video2.gif"), to8b(rgbs), fps=10
            )
    ...
    for i in trange(start, N_iters):
        ...
        rgb, dep, grad, extras = render(
            H, W, rays=[batch_o, batch_d], **render_kwargs_train
        )
        ...
        if args.use_depth:
            ...
            depth_loss = img2mse(
                dep.to(device), torch.tensor(target_depth).to(device)
            )
        else:
        ...
        # extras are the output for coarse model
        if "rgb0" in extras:
            img_loss0 = img2mse(extras["rgb0"], target_rgb)
            loss = loss + img_loss0
            if args.use_depth:
                # print('target_depth shape: ', target_depth.shape)
                depth_loss0 = torch.abs(
                    extras["depth0"] - torch.tensor(target_depth)
                ).mean()
        ...
        if i % args.i_weights == 0:
            path = os.path.join(basedir, expname, scene, "{:06d}.tar".format(i))
            torch.save(
                {
        ...
        if i % args.i_testset == 0 and i > 0:
            if args.stage > 0:
                testsavedir = os.path.join(
                    ...
                    expname,
                    scene,
                    "stage{}_test_{:06d}".format(args.stage, i),
                )
            else:
                testsavedir = os.path.join(
                    basedir, expname, scene, "testset_{:06d}".format(i)
                )
            os.makedirs(testsavedir, exist_ok=True)
            ...
            with torch.no_grad():
                rgbs, _ = render_path(
                    [rays_o_test[-(H * W) :], rays_d_test[-(H * W) :]],
                    hw,
                    render_kwargs_test,
                    savedir=testsavedir,
                    render_factor=args.render_factor,
            ...
            rays_rgb_test = rays_rgb_test.to("cpu")

            gt_loss = img2mse(rgbs[-1], rays_rgb_test[-1])
            gt_psnr = mse2psnr(gt_loss).item()

            ssim_rgbs = torch.unsqueeze(rgbs[-1].permute(2, 0, 1), 0)
            ssim_test = torch.unsqueeze(rays_rgb_test[-1].permute(2, 0, 1), 0)

            ssim_metric = ssim(ssim_rgbs, ssim_test)
            try:
                lpips_metric = lpips(ssim_rgbs, ssim_test)
            except:
            ...
            with open(os.path.join(testsavedir, "statistics.txt"), "w") as f:
                ...
                f.write(
                    "SSIM: {},  LPIPS VGG: {}".format(ssim_metric, lpips_metric)
                )
